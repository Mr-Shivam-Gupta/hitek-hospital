...
public function healthpackages()
{
		...
		$data['depList']=$this->Hitek_Hospital_Model->dbGetDepartment();
		// $data['departmentList'] = $this->Hitek_Hospital_Model->dbDepartmentReport();
		$this->load->view('health-packages',$data);
}
...
public function appointmentData()
{
    ...
    $this->load->view('appointment-data', $data);
				...
				{
					$ndate=explode('-',date('Y-m-d'));
					$numrow=$this->db->get('tbl_apointment')->result_array();
					// $appointmentID ='AP'.$ndate[0].$ndate[1].$ndate[2].count($numrow);
						if ($this->input->post())
						{
							...
							$this->form_validation->set_rules('doctor_id', 'doctor_id', 'required');
							...
							$appointmentDeatails = array(
								// 'appointment_id' => $appointmentID,
							'paitent_name' =>$this->input->post('patient_name'),
							'paitent_email' =>$this->input->post('patient_email'),
							'paitent_number' =>$this->input->post('patient_phone'),
							...
							$query = $this->Hitek_Hospital_Model->dbSubmitAppointment('tbl_apointment', $appointmentDeatails);
							
							if ($query = TRUE)
							{
								$this->session->set_flashdata('success', 'Your appointment submitted successfully and id &nbsp; #');
								return redirect('appointment-data');
							// $id = $this->db->insert_id();
							// echo $id;
							//   echo "1";
							// $data['list'] = $this->Hitek_Hospital_Model->dbGetSubmitedAppointment($id);
							// $data['title']="Appointment  | HI-TEK Super Speciality Hospital";
							// $data['canonical']="https://www.hitekhospitals.com";
							// $data['keywords']="";
							// $data['desc']="";
							// $this->load->view('appointment-data', $data);
							//  redirect('appointment-data',$data);
							}
							else
							{
